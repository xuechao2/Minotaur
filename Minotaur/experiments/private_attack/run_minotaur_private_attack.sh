...
log_path="private_attack_experiment_m0"
mkdir -p $log_path
...
do
    ...
    bash macOS_time.sh
    time=`cat time.txt`
    $binary_path -vv -w 0.5 --betas $bs1 --betaw $bw1 --p2p 127.0.0.1:6000 --api 127.0.0.1:7000 &> $log_path/$betas-$betaw-honest.log --sk c9afa9d845ba75166b5c215767b1d6934e50c3db36e89b127b8a622b120f6721 --ts "$time" &
    pid1="$!"
    $binary_path -vv -w 0.5 --betas $bs2 --betaw $bw2 --p2p 127.0.0.1:6001 --api 127.0.0.1:7001 -c 127.0.0.1:6000 &> $log_path/$betas-$betaw-attacker.log --sk c9afa9d845ba75166b5c215767b1d6934e50c3db36e89b127b8a622b120f3541 --ts "$time" --atttime 120000000 &
    pid2="$!"
