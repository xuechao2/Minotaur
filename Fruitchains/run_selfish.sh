...
binary_path="./target/release/fruitchains"
...
sleep 5
curl -s http://127.0.0.1:7000/tx-generator/start?theta=10000 > /dev/null   # 7000 for 4 nodes; 10000 for 3 nodes
...
curl -s http://127.0.0.1:7001/miner/start?lambda=1000 > /dev/null
# curl -s http://127.0.0.1:7002/tx-generator/start?theta=10000 > /dev/null
# curl -s http://127.0.0.1:7002/miner/start?lambda=1000 > /dev/null
# curl -s http://127.0.0.1:7003/tx-generator/start?theta=7000 > /dev/null
# curl -s http://127.0.0.1:7003/miner/start?lambda=1000 > /dev/null
